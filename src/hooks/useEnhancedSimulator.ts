/**
 * Enhanced Simulator Hook with History and Export
 */
...
export function useEnhancedSimulator() {
    ...
    const runComparison = async () => {
        ...
        try {
            const calculatedSavings = await calculateSavingsWithRetry(invoiceData);
            ...
            // Save to history
            try {
                await saveSimulation(invoiceData, topResults, isMockMode);
                // Refresh history
                await loadHistory();
            } catch (error) {
                console.warn('Could not save simulation:', error);
            }
    ...
    const loadHistory = useCallback(async () => {
        try {
            const simulationHistory = await getSimulationHistory();
    ...
    const exportToPDF = useCallback(async () => {
        if (results.length === 0) {
            setUploadError('No hay resultados para exportar');
            return;
        }
    ...
    const exportToExcel = useCallback(async () => {
        ...
        try {
            await exportResultsToExcel(invoiceData, results);
        } catch (error) {
            console.error('Error exporting to Excel:', error);
            setUploadError('Error al exportar a Excel');
        }
    }, [invoiceData, results]);
