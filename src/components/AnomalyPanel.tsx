...
import { AlertTriangle, AlertCircle, Info, ChevronDown, ChevronUp, Zap, TrendingUp } from 'lucide-react';
                        ...
                        <div key={anomaly.id} className={`rounded-xl border ${cfg.border} ${cfg.bg} overflow-hidden`}>
                            ...
                            {isOpen && (
                                <div className="px-2.5 pb-2.5 space-y-1">
                                    ...
                                    {anomaly.context && (
                                        <div className="flex items-start gap-1.5 bg-slate-100/70 rounded-lg px-2 py-1.5">
                                            <TrendingUp size={9} className="text-slate-400 mt-0.5 shrink-0" />
                                            <p className="text-[10px] text-slate-500 leading-relaxed">{anomaly.context}</p>
                                        </div>
                                    )}
                                    {anomaly.impact && (
                        ...
                        <div key={anomaly.id} className="bg-white dark:bg-slate-800">
                            ...
                            {isOpen && (
                                <div className={`mx-4 mb-3 p-3 rounded-xl ${cfg.bg} border ${cfg.border} space-y-2`}>
                                    ...
                                    {anomaly.context && (
                                        <div className="flex items-start gap-2 bg-white/60 dark:bg-slate-900/30 rounded-lg px-2.5 py-2 border border-slate-200/60 dark:border-slate-700/30">
                                            <TrendingUp size={11} className="text-slate-400 mt-0.5 shrink-0" />
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">{anomaly.context}</p>
                                        </div>
                                    )}
                                    {anomaly.impact && (
                                        <div className="flex items-center gap-1.5">
