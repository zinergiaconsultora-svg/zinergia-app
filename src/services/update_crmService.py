...
# Read the file
with open('src/services/crmService.ts', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the line where saveProposal starts
insert_index = None
for i, line in enumerate(lines):
    if 'async saveProposal(proposal: Omit<Proposal' in line:
        insert_index = i
        break

if insert_index is None:
    print("ERROR: Could not find saveProposal method")
    sys.exit(1)

# New analyzeDocument method to insert
new_method = '''
    async analyzeDocument(file: File, invoiceType?: 'particular' | 'empresa') {
        const timestamp = () => new Date().toISOString();

        console.log(`\\n${timestamp()} 🚀 Starting invoice analysis ${invoiceType ? `(user selected: ${invoiceType})` : ''}`);
        ...
        try {
            ...
            if (invoiceType === 'particular') {
                ...
                console.log(`${timestamp()} 📌 Using PARTICULAR webhook (P1-P3) - user selected`);
            } else {
                webhookUrl = WEBHOOKS.empresa;
                webhookName = 'empresa';
                console.log(`${timestamp()} 📌 Using EMPRESA webhook (P1-P6) - user selected or default`);
            }
            ...
            console.log(`${timestamp()} ✅ ${webhookName} webhook responded`);
            ...
            console.log(`${timestamp()} 📄 Normalized data:`, normalizedData);
            ...
            console.log(`${timestamp()} 🔍 Forensic analysis:`, {
                detectedType,
                detectedTariffType,
                userSelected: invoiceType
            });

            console.log(`${timestamp()} ✅ Invoice data extracted and classified`);
            ...
            console.error(`${timestamp()} ❌ Invoice analysis failed:`, errorMsg);
...
with open('src/services/crmService.ts', 'w', encoding='utf-8') as f:
    f.writelines(lines[:insert_index])
    f.write(new_method)
    f.writelines(lines[insert_index:])

print("✅ analyzeDocument method added successfully")
