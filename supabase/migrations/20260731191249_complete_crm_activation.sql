-- Atomic admin confirmation of activation and its active contract.

CREATE OR REPLACE VIEW public.proposals_alta
WITH (security_invoker = true) AS
SELECT
    p.id, p.client_id, p.agent_id, p.franchise_id, p.status, p.alta_status,
    p.consent_confirmed_at, p.sepa_confirmed_at, p.alta_requested_at,
    p.alta_completada_at, p.alta_rejected_at, p.alta_rejection_reason,
    p.alta_rejection_note, p.alta_requested_by, p.calculation_data,
    p.offer_snapshot, p.current_annual_cost, p.offer_annual_cost,
    p.annual_savings, p.created_at, p.updated_at,
    prof.full_name AS agent_name, prof.email AS agent_email,
    cli.name AS client_name, cli.email AS client_email,
    p.opportunity_id, p.supply_point_id
...
GRANT SELECT ON public.proposals_alta TO authenticated;
