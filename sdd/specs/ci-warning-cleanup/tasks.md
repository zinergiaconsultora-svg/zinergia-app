...
- [x] Task 1: Fix the realtime hook dependency warning in `OcrJobsPanel.tsx`.
...
- [x] Task 2: Remove unused Annual Audit summary parameter.
...
- [x] Task 3: Remove unused Simulator Hero symbols.
  - Traceability: `REQ-001`, `REQ-002`, `REQ-004`.
  - Change: remove `AnimatePresence` import and unused `confirmBtn` JSX local.

- [x] Task 4: Remove unused Aletheia helpers/constants.
  - Traceability: `REQ-001`, `REQ-002`, `REQ-004`.
  - Change: remove `REACTIVE_PRICE_EUR_KVARH` and `nb`.

- [x] Task 5: Verify and close SDD feature.
...
- Removed all six known CI lint warnings.
- `npm run lint` now completes with zero warnings.
- No schema, auth, role, PII, OCR, pricing, commission, public proposal, or integration behavior changed.
